...
export default function MarkdownHistoryDialog({tabId, onClose}: {tabId: string; onClose: () => void}) {
  ...
  const dismiss = () => { if (overview) setOverview(false); else onClose(); };
  ...
  return createPortal(<div className="md-history-overlay" onMouseDown={event => {if(event.target===event.currentTarget)dismiss();}}>
    <div ref={ref} className={`md-history-dialog${overview ? " md-history-dialog--overview" : ""}`} role="dialog" aria-modal="true" aria-label={zh ? "历史版本与草稿" : "Versions and drafts"}>
      <div className="md-history-actions"><strong>{zh ? "历史版本与草稿" : "Versions and drafts"}</strong><div className="md-history-window-actions"><Button pressed={overview} onClick={() => setOverview(!overview)}>{t(overview ? "md.historyExitOverview" : "md.historyOverview")}</Button><Button onClick={dismiss}>{zh ? "关闭" : "Close"}</Button></div></div>
      ...
      {error && <p className="deditor-notice md-history-error" data-tone="error" role="alert">{error}</p>}
      ...
      <div className="md-history-body"><div className="md-history-list">
        ...
        <Button disabled={preview===null || !tab} pressed={compare} onClick={toggleCompare}>{t(compare ? "md.historyContent" : "md.historyCompare")}</Button>
