  ...
  return source.split(/(?<=\n)/).map(part => {
    ...
    if (fenceLength) {
      ...
      if (standardClose && standardClose[1].length >= fenceLength) fenceLength = 0;
        ...
        if (tolerantClose && ticks >= fenceLength && !/^(?:\\`[ \t]*)+$/.test(tolerantClose[2])) {
    ...
    // The visual editor serializes a fence being typed as literal escaped
    // backticks until Enter converts it. Never reinterpret that paragraph as
    // an unclosed code block that consumes the following document.
    if (/^(?:\\`[ \t]*)+$/.test(marker)) return part;
