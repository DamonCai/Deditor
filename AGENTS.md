...
- 2026-09-12 Markdown 输入与长文继续修复：阅读编辑改用原生光标并保留格式边界导航，移除虚拟光标每次输入的强制布局；关闭搜索停止全文扫描，修复 Unicode 搜索偏移，缓存未变文本块、标题位置并避免重复属性写入。见[最新原生光标与长文记录](docs/markdown-native-caret-long-document-2026-09-12.md)。46 核心 / 81 集成 / 142 组件回归、其他完整回归及构建通过，465 项共享样式对照无差异。6303 行浏览器连续输入、保存快照、跨模式撤销通过；303 行原生普通输入、保存与撤销逐字节恢复通过。真实 IME、Windows、大文档原生编辑仍未验收；开发标签发生一次崩溃，冷启动单次未复现，不得计为压力测试全面通过。本执行未提交推送，期间代码被外部操作纳入 `cf1445d`，继续保护独立 XMind 修改。
