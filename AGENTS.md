...
## 紧急并行收尾（2026-09-22，待办剩 8 类）

- 用户剩余37%额度时要求多个agent优先紧急项，三agent分别处理配对、链接卡片、混合文档/多窗口。已关闭链接卡片与混合文档/多窗口两类待办；当前剩8类核查/验收。修复DOM合并普通输入导致闭合符重复、链接编辑丢title、合法file链接被上游清空；取消/并发更新不覆盖新标题，内部事件缓存超限完整回退，不限制正文输入。见[本轮总记录](docs/urgent-closeout-2026-09-22.md)。
- 17配对/10新链接/6原链接/23输入剪贴板/107阅读集成/5近期文件及类型、生产、独立macOS构建通过。原生真实拖选悬浮卡修改/取消/删除/续写保存撤销重开通过；最终包亮暗title/file保存、撤销重做重开及应用内打开、快速配对保存重开通过。双窗recent同步/焦点与Markdown↔XMind独立历史、源字节和ZIP内容逐项校验通过。没有重复全test:all/perf:all，不新增Windows/真实IME/长期内存结论。
- 首次原生批量输入异常仍未捕获异常事件，确定DOM反例与它不能直接等同；具体核查继续保留。测试文件关闭、独立应用退出，未替换日常应用，按既有授权本地提交不推送。历史待办数量以当前pending-tasks为准。

## 格式符、词典与大文档四项处理（2026-09-22）

- 三 agent 并行实现格式符空选区配对/闭合跳过/成对退格、独立应用词典及按文件忽略词；修复配对退格后尾空格与连续输入偏移。优化源 AST/目录上下文复用、首次最终 NodeView 安装及历史行渲染。千节三轮初始化中位 10.70→7.65 秒，标题编辑 1011→300ms；真实万行历史稀疏导航 124→46ms。100KB/1MB 三模式三位置、10 标签冷恢复、300 围栏与主题、千/万行历史及 5000 文件磁盘矩阵补齐。详见[四项整合记录](docs/markdown-four-items-2026-09-22.md)。
- 完整 test:all、最后修复追加 184 项/组相关回归、13 配对/5 词典/3 Rust、新工厂及上下文专项、完整 perf:all、生产和独立 macOS 构建通过。原生词典落盘/重启/文档隔离、逐键配对保存通过。普通包批量输入一次额外星号仍未定因，四次诊断重演通过不抵消该观察；系统拼写红线/菜单仍未形成原生证据。这两条保留在当前待办，不归因工具或 IME，不新增 Windows/真实候选结论。
- 1MB 阅读正文仍有约 234–280ms 输入成本，不称全部场景无卡顿；本次没有增加文件/内容规模门槛。已完成范围从待办移除，剩余 10 类具体核查/验收。自建文档保存关闭、测试应用退出，未替换日常应用；按既有授权本地提交，不推送。

## 当前待办入口（2026-09-22 清理）

- 剩余工作统一查看[当前待办](docs/pending-tasks.md)，只列具体待实现、待优化/补测和待验收动作；完成部分独立存放在[完成记录](docs/completed-tasks.md)。此前“13 组未完成”不表示 13 个功能未实现，链接卡片编辑等已有实现/组件证据，仅补真实操作验收。每完成一个动作即移出相应文字；下方历史“尚未修复/待验收”仅代表当时快照，不能重复开项。
- 本次用户已要求提交前次并行收尾和图块拖动/近期文件的代码及记录；历史“未提交”保留为当时事实，不是当前禁止提交的指令。本次仅本地提交，不推送、不替换日常应用。

## Markdown 性能与本地文件能力检查（2026-09-22）

- 本次从 `c2adcee` 干净工作区检查三模式、编辑操作、搜索/历史/图表与文件打开。阅读行内光标事务避免重复 AST 写回；保留块共享最新 AST 与只读上下文模板，销毁释放、失败可重试；预览万条命中导航只更新前后两项。1000 段光标移动五轮中位 -86.7%，300 节组件初始化三轮中位 -17.9%；千节/千表仍有冷加载与 TOC 更新成本，不称任意规模无卡顿。详见[检查与验收](docs/markdown-performance-audit-2026-09-22.md)、[上下文基准](docs/markdown-context-performance-2026-09-22.md)。
- HTML 修复显式本地 base、srcset、SVG 命名空间 href 和表单目标转换，保留作者 target；打开失败显示中英文原因，失败不记系统近期文件。解除 Mermaid 默认 50k 字符/500 边导致合法图拒绝的门槛；真实亮暗长图/501 边成功。未新增文本大小/扩展名门槛，保留主编辑器不同源隔离，不猜编码后静默破坏文件。未验证边界仍在[当前待办](docs/pending-tasks.md)，不要将本轮通过当全平台验收。

- 最终完整 `test:all`、`perf:all`、前端及隔离 macOS debug 包构建通过；浏览器标题/目录同步、撤销、暗色、万条搜索、HTML 脚本/表单/两种本地图与双语打开错误通过。后续解锁以相同包补齐原生 asset 资源/脚本/表单，阅读保存/一次撤销/重开及目录、引用、脚注上下文更新与精确恢复；六个自建文件哈希一致，三份文档关闭、测试应用退出。临时浏览器页面与服务已关闭，未替换日常应用；不新增千节原生性能、Windows 或真实 IME 结论。产品提交 `5df2aef`，后续只补验收记录。

## 图块拖动与近期文件（2026-09-22）

- 用户新增两项功能：阅读编辑的Mermaid/PlantUML工具栏左侧拖动手柄，落点线/边缘滚动/单次撤销与Alt+↑/↓；同父容器排序保留引用列表，非法/过期/只读/关闭中的移动被拒绝。Command+E（Ctrl+E）近期文件支持搜索、方向键/Enter/Esc、默认上一文件及最多100条跨窗口持久记录。菜单与命令面板有入口，导航/设置弹窗互斥。
- 4轮拖动/5轮近期文件、Rust持久化、147通用/8分隔线、107阅读集成及构建通过。原生实际两类图块上下拖动、保存/撤销原文精确恢复及Cmd+E默认上一项/Enter切换通过；最后补近期文件成功选择后的目标编辑器焦点，最终包重启记录保留、冷打开/已有标签焦点、直接输入保存与撤销原文一致均通过，测试文档关闭、应用退出。最终包范围见[整合记录](docs/diagram-recent-files-2026-09-22.md)。不将浏览器离线PlantUML算远程服务全验，不新增Windows/真实IME结论；未替换日常安装包，未提交推送。

## 并行收尾（2026-09-22）

- 三 agent + 主 agent 收尾见[总记录](docs/closeout-2026-09-22.md)。原生确认并修复 **macOS Cmd+Q 绕过等待写盘、丢失最后可见输入**：AppKit `applicationShouldTerminate:` 接入退出协议，立即输入退出/两窗口恢复逐字节通过。另修关闭取消/快速重试/慢写盘末次输入与已确认窗口锁定，以及延迟粘贴/图片操作在关闭快照后继续写回的缺口。
- 内嵌源码 Cmd+S/Z 字母化已用同一WKWebView纯textarea定位到本机自动化与拼音组词输入链；ABC下普通代码/Mermaid/主源码保存撤销重开通过，不再当作已确认的编辑器快捷键bug。真实中文候选仍未验收。见[原生与异步记录](docs/closeout-shortcuts-2026-09-22.md)。
- XMind 透明/半透明根节点分支穿字已修；280分支/96形状组合/500混合归档/51历史及千节点50步真实浏览器编辑撤销重做通过。原版无theme无填充标题与显式黑色单sheet对照已有新证据，缺省字色未擅自修改；原版视觉、平台和压力边界以[最新XMind记录](docs/closeout-xmind-2026-09-22.md)为准。
- 新增阅读编辑括号/引号/选区包裹独立偏好，以及系统拼写的Alt/Option右键入口。整合test:all、最后107阅读集成/7异步关闭/23输入剪贴板/10窗口生命周期/25图表/3Rust、生产及独立macOS包通过；最终整合包逐键保存/撤销/重做/重开及末字立即Cmd+Q恢复精确通过。系统拼写原生菜单未弹出，仍未验收。测试文档关闭、隔离应用退出、输入源恢复简体拼音。
- Dock实际点击、外部系统拖放、完整跨屏动画、Windows、真实中文候选、真实图床与长期WebView内存仍有未验收边界。不要把历史旧条目的“未修复”或“只剩两项”直接当当前状态。未替换日常安装应用，本次未提交推送。

## 新建编辑器窗口（2026-09-21）

- 新增 macOS Dock / 文件菜单 / 命令面板“新建窗口”，Cmd/Ctrl+Shift+N；窗口独立标签、撤销与恢复状态，空白起步并继承外观偏好。菜单/文件打开/打印/全屏按钮按窗口隔离，关闭和退出等待恢复写盘。原生文件菜单、快捷键、中英亮暗、独立中文输入/保存/关闭/重启恢复、窗口列表通过；5 生命周期/7 持久化/3 Rust 专项、147 通用/8 分隔线及前端、独立 macOS 构建通过。Dock 自动化连接超时，实际 Dock 菜单点击尚未验收；未新增 Windows/真实 IME/多窗口全屏动画结论。见[新建窗口记录](docs/new-editor-window-2026-09-21.md)。未替换日常安装包；按用户要求提交，不推送。

## HTML 动态预览修复（2026-09-20）

- 后续按用户“本地编辑器不应过度限制”检查并修复：移除编辑器自加 CSP/refresh 过滤，HTML 放开正常表单/弹窗/下载；Markdown 动态 HTML/页面样式改为独立文档，保留本地 iframe/srcdoc/全屏属性并增加源码入口；补媒体、SVG资源路径和已挂载 source 的按需加载，避免音视频控件点击误入源码。完整 test:all 通过，最后媒体/SVG修改另过4预览/2复用/8特殊块及构建；真实脚本/本地嵌入/音视频播放/亮暗中英窄窗/编辑撤销与静态SVG通过。保留主编辑器不同源隔离，未新增原生验收或替换安装包；按用户要求补入本地编辑器不过度限制的协作规则，与修复一并提交，不推送。详见[限制检查记录](docs/preview-restrictions-audit-2026-09-20.md)。本条替代下条的额外 CSP 限制。

- 用户提供的 `gateway-traffic-flow.html` 空框原因是 HTML 预览完全禁止脚本。现允许脚本在无同源权限的独立 iframe 内执行，继续限制顶层导航、嵌套 frame、表单和 eval；原文件未改。真实浏览器图形/字段/页签/缩放、窄窗分栏/双实例/修改撤销及隔离检查通过，147 通用/8 分隔线与生产、独立 macOS 构建通过。原生 UI 工具错误/超时，不能计作原生验收；测试包进程已退出，未替换日常安装应用。执行中外部提交 `16efb34` 纳入产品修复，本任务未提交推送；详见[验证记录](docs/html-script-preview-fix-2026-09-20.md)。

## XMind 接续（2026-09-22 清理）

- 图片资源迁移/同名避让、所选主题间联系和内部链接复制已修复，浏览器、macOS 保存/撤销/重开及原版读取已闭环；Review24 大附件重开/原版读取也已补齐。见 [剪贴板记录](docs/xmind-clipboard-fix-2026-09-12.md)。不再作为待办。
- 透明/半透明根节点分支穿字已修；96 形状组合、280 分支检查、500 次混合归档、51 历史与千节点 50 次编辑/撤销/重做通过。无 theme 无填充标题差异已定位为原版缺省字色差异，保留 DEditor 可读默认色。剩余原版精确视觉规则、长期压力、最新整合跨格式原生交互及平台验收见 [XMind 当前待办](docs/xmind-pending-audit-2026-09-12.md)；证据见 [最新收尾记录](docs/closeout-xmind-2026-09-22.md)。
- 用户已授权持续对照原版、修复并验证。批次通过不代表整体完成，单工具不可用时继续独立可做项；不把未验证计为通过。历史轮次保存在 [跨机器接续档案](docs/xmind-handoff-2026-09-11.md)。
- 原生验证仅用自建样例，结束前保存并关闭测试文档，立即退出 DEditor / XMind 测试实例，避免恢复窗口积累。保护并行改动，接续先查 Git；`tests/artifacts/` 被忽略，跨机另行迁移证据。

---

## Markdown 可视化编辑架构（2026-09-11，核心接入已实施）

- 2026-09-21 全局搜索跨模式横查：补实时预览右侧的指定命中高亮和居中，HTML／CSV 纯阅读搜索展开源码双栏以展示属性/转义文本命中，取消过期模式请求。11 全局导航/12 搜索/107 阅读集成/147 通用/8 分隔线/6 分屏/5 CSV 与生产、隔离 macOS 构建通过；亮暗中英宽窄浏览器及原生自建 Markdown/HTML/CSV 搜索通过，4 文件字节一致、测试应用退出。最后滚轮释放逻辑仅另过搜索、浏览器和生产构建。见[跨模式检查](docs/global-search-modes-audit-2026-09-21.md)，含源码特有匹配和首尾行边界；未替换日常应用，未新增 Windows，按此前授权提交。

- 2026-09-21 用户澄清重新打包后、搜索前即阅读编辑仍无高亮：本次补全局搜索独立命中装饰，不再仅靠原生选区；正文、展开/收起代码均橙色显示，失焦与原生选区清空保持，正文交互/修改清理。6 全局搜索/12 搜索/107 阅读集成/147 通用/8 分隔线与生产、独立 macOS 构建通过；实际原生首次新标签代码 quickstart 与暗色正文命中居中高亮通过，测试文档关闭、应用退出。见[阅读显示补充](docs/global-search-first-open-2026-09-21.md)末节。未替换日常应用，未新增 Windows 验证，按用户要求提交。

- 2026-09-21 全局搜索首次打开无高亮修复：移除 120 帧定位截止，改为等待目标编辑器就绪；定位跨越搜索弹窗卸载，离开文件/切换模式/重新搜索取消旧请求。5 新增全局搜索/12 搜索/147 通用/8 分隔线及构建通过；真实亮暗中英、720px/1280px、首次正文/长代码/源码及慢加载居中通过。见[首次打开搜索修复](docs/global-search-first-open-2026-09-21.md)。按用户要求提交，未重打原生包。

- 2026-09-20 图表全览按用户要求实施：Mermaid/PlantUML 工具栏增加全览，顶层窗口复用原有三模式和源码编辑器，支持缩放/100%/适应窗口/Ctrl或Cmd滚轮；Esc、模式切换、菜单与其他弹窗焦点交接、删除/撤销和后台清理已覆盖。24 图表/11 历史/107 阅读集成/146 通用/8 分隔线及生产构建、亮暗中英宽窄真实浏览器验证通过。见[图表全览记录](docs/markdown-diagram-overview-2026-09-20.md)；本次与上一条图框自动高度一起按用户要求提交。未重打原生包，PlantUML 布局用离线自建 SVG，不计远程服务验收。

- 2026-09-20 用户明确图框“随图自动增高、取消框内上下滚动”：Mermaid/PlantUML 阅读及块内实时预览移除 60vh 上限与首次切换高度锁定，短图收紧，实时预览按图高保持左右等高；PlantUML 内联高度在窄窗保持比例。此要求替代此前固定图框高度。18 图表/146 通用/8 分隔线与生产构建通过，真实亮暗中英/720px 长短宽图、分栏调整、源码编辑撤销、空图/错误恢复通过。见[图框自动高度记录](docs/markdown-diagram-autosize-2026-09-20.md)。PlantUML 布局用离线自建 SVG，不计远程服务验收；未重打原生包、未提交推送。

- 2026-09-20 按用户澄清保留整窗全览，并在“查看版本内容”旁增加左右内容的独立“对比全览”；Esc/关闭逐层返回历史窗口，版本/折叠/差异位置及入口焦点保留。Command+Shift+F 的结果点击/Enter/空格后收起弹窗，源码及阅读模式选中匹配并居中，长代码块按内嵌实际选区校正。11 历史/12 搜索/107 阅读集成/146 通用/8 分隔线与生产构建通过；自建 160 段及 60 行代码样例在亮暗中英/宽窄真实浏览器核对，目标中心偏差小于 1px、原文一致。见[独立全览及搜索记录](docs/markdown-history-overview-2026-09-20.md)末节；未重打原生包、未提交推送。

- 2026-09-20 历史对比按“相同内容尽量折叠”调整：默认开启折叠，差异两侧各留一行上下文（替代首版三行）；完全相同版本整段折叠，可逐段或全部展开。换版本恢复默认折叠，全览往返保留当前状态。11 历史专项/7 会话/145 通用/8 分隔线与生产构建、真实中英亮暗宽窄窗口验证通过。见[历史默认折叠记录](docs/markdown-history-overview-2026-09-20.md)末节；未重打原生包，未提交推送。

- 2026-09-20 历史全览后续按用户反馈改为占满应用窗口、无外部留白；全览中 Esc/关闭仅返回普通历史弹窗，保留版本、折叠及差异定位，方便继续选版本。1280px 中文亮色/720px 英文暗色及分层退出连续操作通过，9 历史专项/145 通用/8 分隔线与生产构建通过。见[全览后续交互记录](docs/markdown-history-overview-2026-09-20.md)末节。本次后续未重打原生包，勿将首版 macOS 结果算作后续验收；未提交推送。

- 2026-09-20 历史版本/草稿新增全览、全文差异分布点击定位、折叠未变更内容（三行上下文与单段展开）、上一处/下一处差异和序号。切换版本重置导航；全览/窗口缩放保持当前差异可见，单段展开不跳回旧差异；补长错误滚动和低窗口布局。9 历史差异/7 会话/145 通用/8 分隔线及生产、独立 macOS 构建通过；真实中英亮暗/720px/键盘与原生版本/草稿导航、恢复保存/单次撤销/文件面板重读字节核对通过，测试应用和服务退出。见[历史全览验证](docs/markdown-history-overview-2026-09-20.md)。未提交推送、未替换日常应用；本次未跑完整 test:all，未新增 Windows/真实 IME 验收。

- 2026-09-19 表格/图表五项反馈已实施：表格四向批量插入 1–100、水平列对齐和垂直单元格对齐（隐藏注释保留，预览/富文本往返一致）；修复从下方段落向上拖选被表格插件收缩；图表淡色工具栏、名称选块和独立删除按钮；阅读/图表源码/实时预览右键菜单跟随中英文。完整 test:all（33 表格/20 剪贴板/107 集成等）通过；后补异步粘贴保护与普通代码菜单焦点后 18 图表/145 通用/8 分隔线及生产、macOS 构建通过。浏览器亮暗/720px/键盘、原生批量行列/对齐/拖选/图表删除/保存撤销重开精确核对通过，测试应用及临时页面服务已关闭。原生自动化向内嵌源码发送保存组合键仍曾输入字母 s，未定因、未计为快捷键通过，已通过应用按钮恢复；Windows/真实 IME 未新增验收。见[五项反馈记录](docs/markdown-table-diagram-ux-2026-09-19.md)。未提交推送，未替换日常应用。

- 2026-09-19 按用户“加上”补齐表格选列/选表/四向插入/清空、阅读工具栏 Markdown/富文本/Excel TSV 复制及纯文本粘贴、历史版本差异和 `/` 筛选插入；修复末格 Tab 增行与随后的输入合并撤销。表头前插行保留合法结构，富文本默认对齐往返不再自动加冒号。完整 test:all 通过；最后默认对齐修正后 19 剪贴板/29 表格/107 集成/145 通用/8 分隔线及构建通过，7 会话/菜单/历史、9 Enter 历史通过。浏览器亮暗/中英/720px及 macOS 自建样例逐键、复制粘贴、历史恢复、保存撤销重开通过；最终包富文本全文往返原文精确一致，测试应用退出。见[功能补齐验证](docs/markdown-editing-features-2026-09-19.md)。未修改用户损坏文件，未新增 Windows/真实 IME/跨应用 Excel或钉钉验收，未提交推送。


- 2026-09-19 表格右键菜单按用户截图重新调整：13px 字号、28px 行高、左对齐、小图标及选择/删除分隔线，独立于正文字号；只作用于菜单，动作及中英文名称不变。真实亮暗/中英文/720px/放大正文、键盘焦点和删除撤销/只读通过；28 表格、145 通用/8 分隔线及生产构建通过。见 [菜单外观记录](docs/markdown-table-menu-style-2026-09-19.md)。未重新打原生包或替换安装包，未提交推送。

- 2026-09-19 默认表格列对齐统一：工具栏建表为 null，表格内加列上游默认 left，导致后者自动出现冒号；现将单元格/表头默认值设为 null，显示仍靠左，保留显式左/中/右对齐。28 表格、107 阅读集成、15 剪贴板、8 连续表格、145 通用/8 分隔线及生产构建通过；真实浏览器加列按钮、混合对齐、撤销及重新解析通过。见 [默认列对齐记录](docs/markdown-table-default-alignment-2026-09-19.md)。未重打原生包、未修改用户文件、未提交推送；此前 H-TAB-01 未修复。

- 2026-09-19 用户反馈添加 PlantUML 后表格保存损坏：已复现完全转义的围栏输入被容错解析误认成代码块，局部 AST 吞入引用上下文，后续逐字输入截掉相邻表格源码。现保留转义文字并对四条局部投影路径限定块范围；26 表格/32 代码行数及新增转义兼容断言、145 通用/8 分隔线与生产、独立 macOS 构建通过。34 项分类回归 33 通过，唯一失败仍为此前 H-TAB-01（Tab 增行后立即输入的撤销边界），未混同修复。浏览器亮暗与 720px、原生逐字语言输入/真实保存/撤销重做/关闭重开/表格续写字节核对通过；原生反引号因键盘布局用逐字符粘贴，详见 [围栏表格保存修复](docs/markdown-fence-table-save-fix-2026-09-19.md)。用户原文件只读并留副本，期间有外部修改，不能称哈希未变；测试应用已退出，未提交推送，未替换日常安装包，未新增 Windows/真实 IME 验收。

- 2026-09-18 用户要求从操作现象提取类型并据此测试：已整理 10 类操作及交叉维度，统一入口 `npm run test:markdown-operations` 按类型关联 34 个执行项、失败继续并保存逐项报告；新增整篇复制/剪切/粘贴及结构操作快慢对照。发现 **H-TAB-01：表格末格 Tab 增行后立即输入，撤销会连新增行一起撤掉；停顿后只撤文字**，DOM 和真实浏览器均复现，列表缩进/反缩进及引用拆分快慢对照通过。该断言保留失败，本轮是测试审计、未修产品代码，不能称全绿。最终结果和平台边界见[按类型测试方案与记录](docs/markdown-operation-test-plan-2026-09-18.md)。没有提交推送。

- 2026-09-14 HTML 围栏显示补充：用户最终 5,865 字节 SVG 使用 28 个未定义的 Tailwind 风格变量，修复前 15 个矩形中 12 个黑色，现补齐 slate/indigo/emerald/sky/rose/amber 后逐节点黑色与未解析值均为 0；`fill="none"` 以白色默认画布显示。无显式高度的响应式围栏 SVG 按内容收紧空轴，源码不改。`html/HTML/Html/hTmL`、转义大写围栏、大小写混合标签均通过。纯 HTML/图表跳过 Shiki，已连接 SVG 同步测量；当前文件冷解析 33.55→1.31–1.43ms，真实页首屏 346.70→95.20ms，流水线 165→9.3ms。另修复光标记忆导致阅读编辑首次自动展开源码：源码切换、标签切回均直接展示 SVG，实际点击才编辑。最终完整 test:all、perf:all、macOS 构建及当前用户文件亮暗原生验证通过，测试应用退出；旧隔离包曾追加 6 个文末空行，已按测试前副本恢复，最终哈希一致。详见 [HTML 围栏记录](docs/markdown-fenced-html-render-2026-09-14.md)，未提交推送。

- 2026-09-14 HTML 围栏渲染已修复：兼容标准 ` ```html ` 及富文本复制形成的拆分/转义围栏；实时预览和阅读编辑共用安全清理后的实际 HTML/SVG 展示，点击仍可编辑源码并保持原始围栏。用户提供的 `/Users/damon/Documents/fy_report/测试.md` 已只读验证，亮/暗、宽屏/720px、源码编辑撤销和 macOS 两模式通过，原文件 SHA-256 前后一致，测试应用退出。自动化覆盖脚本/事件清理、原文保真和 DOM 复用；当前整合工作区完整 `test:all`、62 核心/107 阅读集成/145 通用/8 分隔线及生产/macOS 构建通过，见[HTML 围栏渲染记录](docs/markdown-fenced-html-render-2026-09-14.md)。未新增 Windows 验收，未提交推送。

- 2026-09-14 表格普通 Enter 跳出：已覆盖上游 ExitTable 的普通 Enter 绑定，改为当前单元格内换行，保留 Cmd/Ctrl+Enter 退出；矩形选区 Enter 转入单格编辑。24 表格/8 连续表格/8 Enter 历史/107 集成/145 通用/8 分隔线及生产、macOS 构建通过，真实浏览器亮暗与720px、原生逐键/保存/撤销/重做/关闭重开精确核对通过。见[回车修复记录](docs/markdown-table-enter-fix-2026-09-14.md)。测试应用已退出，未提交推送，未新增 Windows/真实 IME 候选验收。


- 2026-09-14 用户继续反馈实时预览输入卡顿：已用约 4,470 行自建复杂文档在 macOS 包采集，修复输入时整页 DOM／未改图表重复生成。新增逐块共享清洗缓存、DOM 复用和独立行号映射；三次同位置采样 DOM 提交中位数 104→9ms，HTML 处理 115→61ms，最大采样帧间隔 4266→449ms（含原生采集开销，非纯按键延迟）；整篇解析约百毫秒仍保留。原生逐键、连续十键、换行删除、保存与撤销精确核对及主题最终显示通过，测试应用退出。见[卡顿实施与验证](docs/markdown-preview-latency-2026-09-14.md)。固定副本完整 test:all、perf:all、生产/macOS 构建通过，采集入口不进入最终包；保护并行分栏/搜索等修改，不将固定副本结果概括为最新工作区全量验收。本任务未提交推送，未新增 Windows、完整 IME 或长期运行结论。

- 2026-09-14 阅读表格右键及 Shift+F10 菜单增加“删除列”，支持当前列、多列和末列移除表格，单次撤销；补齐列选区键盘菜单定位。22组表格、通用回归与生产构建通过，真实浏览器亮色中列/暗色720px末列删除及撤销原文一致。见[删列记录](docs/markdown-delete-column-2026-09-14.md)。未新增原生包验收，未提交推送；此前钉钉富文本复制讨论尚未实施。

- 2026-09-14 IDEA 式向右分屏已实施：左右独立标签/模式/光标/滚动、同文件共享正文与撤销、拖动调宽、关闭空组及合并保留文件。原生验收发现辅助功能点击跨栏工具栏未激活面板，已补 click/contextmenu 捕获与最新 bridge 焦点恢复。见[分屏实施与验证](docs/split-right-2026-09-14.md)。固定副本完整 test:all 通过，最后点击修正后 6 分屏/107 阅读集成/145 通用及 8 分隔线与 macOS 构建通过；原生双文档逐键/跨栏撤销重做/分别保存/关闭重开精确核对通过，测试实例已退出。最后当前整合工作区 6 分屏/107 阅读集成及生产构建也通过。固定包以已验证搜索基线排除并行搜索修改，不代表最新整合原生通过；未新增 Windows/真实 IME/原生快捷键结论，未提交推送。

- 2026-09-14 阅读编辑搜索（用户纠正后的最终版）：撤回新建共享面板及改写原有搜索样式，源码恢复CodeMirror默认搜索，阅读直接挂载库自带页脚面板并适配阅读操作。1280px单行/720px暗色换行与现有组件全部控件位置、尺寸、字体、间距一致；保留Enter定位、高亮与主题恢复。见[最终搜索修复记录](docs/markdown-search-footer-fix-2026-09-14.md)。10搜索/107集成/145通用与8分隔线、13基本/6会话/8焦点及构建通过；未新增原生包/真实IME结论，不代表并行分屏等最新整合通过，未提交推送。此前200px输入宽/66px双行新设计已作废，不要恢复。

- 2026-09-14 按用户要求用自建 4,469 行复杂文档打包原生验证：旧包第 246 行输入后画面跳回第 1 行，现区分预览主动滚动与重渲染自动滚动，修复后同处输入/换行/删除保持位置，第 2,152 行逐键输入及撤销保存逐字节核对通过。阅读列表续段/标题新增减少缩进按钮、Shift+Tab 与标题行首退格，横向补连续标题边界；最终原生普通/引用/任务/连续标题及保存重开通过。见[原生滚动与顶格记录](docs/markdown-native-scroll-outdent-2026-09-14.md)。最终 17 格式组/107 阅读集成/24 任务及生产/macOS 构建通过，此前本次 12 预览/62 核心/145 通用/8 分隔/7 导航通过；不是 Windows 或完整 IME 验收。测试文档保存关闭、独立应用退出，用户原文件未改。本任务未提交推送，期间外部提交到 `4a5c9b7`；保护后续并行 App/EditorGroups/EditorPane/store 改动，不把本次包结果归于它们。

- 2026-09-13 用户提供真实文件核对表格：已修复列表标记后 NBSP、转义标记加 NBSP、中文标点粗体标签及嵌套 `<br>` 解析，并补虚拟有序列表元数据防止打开时改写原文。见[表格导入格式修复](docs/markdown-table-import-format-2026-09-13.md)。原始副本两模式均为 52 列表项 / 9 粗体；28 预览 / 19 表格操作 / 62 核心 / 107 集成 / 145 通用 / 8 分隔线 / 47 语法及生产和 macOS 构建通过。自建原生样例查看、保存、一次撤销逐字节恢复和关闭重开通过，测试应用已退出；未修改用户原文件，未提交推送，保护并行标签切换优化。用户文件期间有外部追加，详见记录，不能称整文件哈希未变。

- 2026-09-13 顶部文件切换优化：阅读编辑由单个 keyed slot 改为最多8个实际访问文件的 MRU 复用；源码专用/HTML/比较标签保持惰性。真实600段浏览器暖切换中位268.45→57.7ms（约-79%）。原生初版发现隐藏文件仍接收系统撤销，已补换文件焦点交接与隐藏事件保护，最终包两文件逐键输入/切换立即撤销/重做/保存/关闭重开逐字节核对通过，测试文档关闭且应用退出。完整test:all及焦点修正后的107集成/145通用/8模式焦点/7新专项通过，生产/macOS构建通过；最终固定副本完整perf:all通过（9月14日凌晨），结果见[切换性能记录](docs/markdown-tab-switch-performance-2026-09-13.md)。本任务未提交推送，期间外部提交到0d7dd0f包含初版；保护后续并行表格格式修改，固定原生快照不代表这些并行改动通过。未新增Windows/真实IME验收。

- 2026-09-13 表格选列相邻空白误高亮：macOS复现列表列选区使左侧空白变蓝，实际复制仅含目标列；原因是Crepe文字选区色覆盖隐藏规则。仅恢复阅读表格隐藏原生选区样式，修复包亮暗/正反拖选/顶部整列选择通过，清空两格保存与一次撤销逐字节恢复通过。18表格/145通用/8分隔线及生产/macOS构建通过，测试应用退出。见[选列修复记录](docs/markdown-column-selection-fix-2026-09-13.md)。本任务未提交推送，外部提交已包含本批代码；保护并行链接修改。未新增Windows或修复包重开验收，列表样例初始列宽格式化属既有独立边界。

- 2026-09-13 文件链接修复：实时预览解析/清理层及阅读 schema 保留 file href，阅读悬浮卡片接入应用内打开；file URL、无协议绝对、同级/上级相对路径均沿源文档目录处理。见[文件链接记录](docs/markdown-file-links-fix-2026-09-13.md)。14格式链接/145通用/8分隔线/107阅读集成及生产、macOS构建通过；原生自建文件实时预览四种路径、阅读实际悬浮卡片file/绝对/上级相对打开均通过，源文档保存原文一致，文档关闭、隔离应用退出。未新增Windows验收。本任务未提交推送，外部提交dd75fa2已包含本批与并行表格修改，接续重查Git，保护并行工作。


- 2026-09-13 用户要求 Mermaid/PlantUML 块内阅读居中：仅成功图形容器增加阅读模式水平居中，亮色大屏、暗色720px与模式往返中心偏差均0px、原文一致，14图表/145通用/8分隔线及构建通过。见[图表居中记录](docs/markdown-diagram-centering-2026-09-13.md)，未新增原生包验收。此前主实时预览分隔线误选已补指针捕获及临时选择保护，浏览器亮暗/窄窗与回归通过，但本轮macOS坐标拖动未形成可靠闭环（最后一次布局变化且出现选区），不得记为原生通过，后续须核实命中位置与事件；自建原文一致，测试文档关闭、隔离应用退出，见[分隔线记录](docs/markdown-preview-divider-fix-2026-09-13.md)。本任务未提交推送，期间外部提交到ec079a6，接续重查Git并保护并行工作。


- 2026-09-13 用户五项反馈修复：阅读/实时预览在线面板解除 860px 上限并随侧栏/窗口伸缩；表格新增右键与 Shift+F10 “选中整行/删除行”，覆盖表头提升、仅表头、多行及单次撤销；修复上游延迟段落选区及装饰更新重建整表；Mermaid/PlantUML 增加包含工具栏的完整边框并移除复制。见[五项反馈实施与验证](docs/markdown-reading-feedback-2026-09-13.md)。完整 test:all（62 核心/107 阅读集成/145 通用/119 XMind、17 表格/14 图表）和生产/macOS 构建通过；实际宽窄/亮暗、正反拖选和中文完整复制通过，原生窗口 zoom/侧栏收起、删行保存/一次撤销逐字节一致/重做保存关闭重开通过，隔离测试应用已退出。完整 perf:all 通过；100 行表格 30 次选区更新独立专项中位 381.8→12.05ms（全套末尾 59.65ms）、整表重建 30→0（jsdom 局部指标，不代表全场景卡顿消失）。最终宿主样式作用域保留导出原列宽；Windows/IME 未新增验收。未提交推送；不是此前整体 Markdown/XMind 路线图全部完成。


- 2026-09-12 用户明确要求暂停、更新上下文下次再做：三agent操作缺口检查新增7类修复（删除历史、空占位退出、保留块剪贴板、快速表格拖放、目录点击/焦点、保存失败反馈、弹窗后内嵌焦点），详见[本轮结果与暂停接续](docs/markdown-operation-gaps-results-2026-09-12.md)。最终范围明确副本完整test:all、62核心/107阅读集成/145通用/119XMind及33专项、生产/macOS构建通过；不含并行图表最新开发。最终包原生快速a/b/c后Cmd+Z/S插入字面z/s，原因未明，优先续查，不能计为通过或擅自归因IME/工具。较早包真实只读保存失败/粘贴续写/重试/重开通过，不替代最终包逐键验收。自建现场已保存归档，文档关闭、测试应用退出。真实悬浮、持续跨屏拖选、原生跨应用剪贴板、动态高度/系统缩放、IME/Windows等仍待验收。外部操作已提交到ccd6a14包含本批代码，本任务未执行提交推送；接续重新检查Git并保护并行图表/XMind。证据位于被忽略的tests/artifacts/operation-gaps-handoff-2026-09-12，跨机另行迁移。按用户要求暂停，不因本轮通过宣称整体完成。

- 2026-09-12 图表外观再调整：模式按钮加宽至最少72px、左右留白18px；三种模式统一工具栏占位，首次切换记录内容区高度，后续切换不改变外框或下方正文位置，长内容内部滚动。亮色Mermaid六次切换、长内容及撤销、暗色720px PlantUML五次切换的实际尺寸一致；12图表专项、145通用与固定副本构建通过。见[图表最新外观记录](docs/markdown-diagram-modes-2026-09-12.md)。未新增原生包、未提交推送。 随后已修复阅读切换误触发整块选中蓝框，保留统一按钮底色与键盘焦点；13组图表专项及亮暗实际点击验证通过。 用户随后要求以右上角为准，已取消72px最小宽度、改回左右10px留白；真实同页参照测得44/69/45px宽、22px高完全一致，此前扩大尺寸已作废。

- 2026-09-12 图表交互后续：按用户反馈统一为「编辑 / 实时预览 / 阅读」，实时预览去掉两侧独立框线、使用等高左右区域及可拖动单中线，支持键盘微调，窄窗维持左右分栏。12组专项、145通用与固定副本构建通过；真实浏览器拖动、长图等高滚动、暗色720px、模式保留比例及撤销原文核对通过。见[图表记录最新部分](docs/markdown-diagram-modes-2026-09-12.md)。未新增原生包，未提交推送；首版上下排列与旧名称已被本条取代。

- 2026-09-12 用户明确图表采用「编辑 / 编辑+预览 / 纯展示」：阅读编辑 Mermaid / PlantUML（含别名）已增加块内三模式、实时双栏、窄窗上下排布、空源码提示及过期渲染保护。见[图表三模式记录](docs/markdown-diagram-modes-2026-09-12.md)。9组专项、4特殊块、12代码外观、145通用与固定副本构建通过；真实浏览器两类图表更新/撤销、亮暗/窄窗、错误恢复和键盘入口通过。完整阅读集成仍有混合列表换行失败，替换回原版图表代码也失败，其余106项通过；未新增原生/IME验收，未提交推送，保护并行修改。

- 2026-09-12 目录外观后续：用户要求未固定时也与固定外观一致；共享 CSS 已统一展开面板为 240px、贴顶整栏、淡色背景／左边框／无圆角阴影，宽窄窗均一致，仅保留悬浮收起与固定占位行为差异。见[目录记录末节](docs/markdown-outline-menu-fix-2026-09-12.md)。浏览器宽窄／亮暗实测和2组目录集成通过，本次仅 CSS，未新增原生包。

- 2026-09-12 用户要求阅读目录以预览为准，并将查找替换放入原生“编辑”菜单：两侧已共用预览的悬浮／固定目录与高亮规则，删除阅读右上角查找／目录按钮及旧过滤折叠侧栏；修复预览中文目录 ID 编码匹配，菜单 Cmd/Ctrl+F、Cmd/Ctrl+Alt+F 分别打开查找／替换。见[目录与菜单修复](docs/markdown-outline-menu-fix-2026-09-12.md)。107 阅读集成、144 通用、5 搜索／7 导航／19 边界、生产与 macOS 构建通过；浏览器亮暗各19目录条目样式一致，原生菜单实际点击、替换保存／一次撤销逐字节恢复、源码替换焦点、目录固定／同名末节／关闭重开通过。隔离测试应用已退出，未新增 Windows／IME 结论。本任务未提交推送；期间外部提交到 `21d5d13`，随后外部调整 1.0.0 版本号，继续保护这些修改；本次原生验收是独立 0.9.0 包。


- 2026-09-12 用户反馈文末代码块难以删除／换行，已复现点击下方空白却在代码前插入空行，以及最后行中间 ↓ 无法退出。已补文末点击落点、代码显示行边界退出、Cmd/Ctrl+Enter、鼠标整块选择及选中块 Enter，详见[文末操作修复](docs/markdown-document-end-fix-2026-09-12.md)。19 组边界、62 核心、106 阅读集成、8 Enter 历史、9 特殊块连续操作、12 代码显示及生产／macOS 构建通过；真实浏览器逐键、删除撤销原文一致、亮暗与折行检查通过。原生系统捕获连续报错，未完成新包原生保存／撤销／重开，不能计为通过；隔离测试进程已退出，未提交推送。


- 2026-09-12 按“bug修改”漏检复盘继续三个 agent 并行：见[36 条连续操作清单](docs/markdown-operation-sequences-2026-09-12.md)及[第三轮整合结果](docs/markdown-operation-round3-2026-09-12.md)。新增14类修复，本任务三轮去重合计47类（不重复计算独立任务列表/代码外观修复）；40组新增专项、最终固定副本完整 test:all、62核心/106阅读集成/144通用/119XMind及生产构建通过，亮暗各1374稳定呈现一致。真实逐键曾拦下初步修复 `N**EXTword**` 和删词撤销只恢复首字，已补齐逐字检查并重新完整验证；不能再用整词一次插入冒充逐键。链接取消/确认后续写通过，但浮层由标记测试按钮调用真实API激活，悬浮入口未验证。任务列表独立原生包不含本轮全部改动；本轮未新增原生包、真实IME、Windows、图床或长期压力结论。测试标签/服务已清理，未提交推送，保护并行改动。

- 2026-09-12 用户要求落实后端优化：已增加有界历史正文缓存及保存前旧文读取上限；状态落盘抽为可直接验证的模块，保持原解析和保存顺序。见[后端实施记录](docs/backend-performance-implementation-2026-09-12.md)。最终 Rust 30 项回归、351 项语义对照及 8 场景性能对照通过；20 个 1MiB 文档重复历史核对 2.475→0.589ms。RawValue 定向解析实测退化，已撤回，不能称全量状态解析/IPC 已优化。缓存收益仅 macOS 实测，Windows 保守回退；没有新增原生 UI/IME 验收或提交推送，保护并行前端修改。

- 2026-09-12 用户反馈空已完成父任务不能退格、连续 Tab 不缩进，已复现 `<br>` 占位与第一项缩进限制并修复：任务退格/回车退出、连续正反缩进、多选/子树、行末 Delete 合并及独立撤销；修正嵌套负边距。24 组任务 / 13 基本 / 7 导航、106 阅读集成及构建通过；独立 macOS 完整副本真实按键、保存、撤销逐字节恢复、缩进关闭重开通过，文档关闭且测试应用已退出。见[任务交互修复记录](docs/markdown-task-interaction-fix-2026-09-12.md)。首条任务独立缩进用隐藏 HTML 标记保存，其他编辑器不保证显示该附加缩进；现有空 `<br>` 纵向高度差未纳入修复。原文未改，无提交推送；固定原生快照不代表随后并行修改整体通过，未新增真实 IME/Windows 验收。

- 2026-09-12 第二轮按用户要求三个 agent 并行继续，新增 7 项修复、两轮去重合计 33 项。接手先读[第二轮结果](docs/markdown-operation-round2-2026-09-12.md)：保存等待期间失效、插入弹窗 store/view 不同步、表格跨块重做增行、非键盘 insertText 跨块带入显示色、三类特殊块出口。固定快照 62 核心 / 106 阅读集成 / 163 操作专项、144 通用 / 119 XMind 及其他分项通过；test:all 末项因副本漏 public 失败，补齐后图标通过，最终完整资源原生构建通过。亮暗各 1374 呈现无差异。原生代码首部退出/保存/一次撤销精确通过；随后窗口被用户操作并出现非测试文档，未强退应用，自建 HTML 标签未编辑但未能安全关闭，其余原生未完成。保护并行性能/代码块显示改动；固定副本排除了其进行中部分，不代表最新工作区整体通过。未提交推送，真实 IME/Windows/拖拽/图床/长期压力仍未全面验收。


- 2026-09-12 用户要求代码块点击后保持原状：已统一编辑/预览的 Shiki 高亮、字体、行号和换行，取消点击焦点框与浮层，语言/复制控件改由右键或键盘聚焦显示。见[代码块点击外观记录](docs/markdown-code-focus-fix-2026-09-12.md)。真实浏览器点击、亮暗/窄窗/空行、输入与原文撤销通过；12 专项、106 阅读集成、32 行数、8 Enter 历史、4 特殊块组及构建通过。未重打原生包、未提交推送，保留并行边界/性能等改动。

- 2026-09-12 用户反馈代码围栏 Enter 后撤销结果随快慢变化：已稳定复现 750ms 分组吞掉语言输入，补齐 Enter 操作边界、结构／选区历史及撤销移除内嵌编辑器后的原生焦点交接；同时修复文末空段落和末尾 Shift+Enter 未记录。见[Enter 撤销修复记录](docs/markdown-enter-history-fix-2026-09-12.md)。最终 8 组专项 / 106 阅读集成、此前完整回归与生产构建通过；独立 macOS 最终包快慢 Enter、撤销续写、重做代码输入及连续空行均实际按键、保存原文核对通过，测试文件恢复后关闭并退出。真实 IME / Windows 未新增验收；不表示其他基本操作已全部无缺陷，未提交推送，保护并行表格／格式／粘贴修改。

- 2026-09-12 用户要求依据 64 组操作清单逐项验收，并明确授权三个 agent 并行。接手读[逐项验收记录](docs/markdown-operation-verification-2026-09-12.md)文首及专项引用：任务父状态、代码/列表 Tab、表格换行/空白/菜单、格式状态、替换保真/前进、图片属性撤销/多图粘贴、草稿及关闭持久化、冷启动路径队列等已修复。v5 固定快照 62 核心 / 106 阅读集成 / 94 组操作专项、完整回归和原生构建通过；原生保存/撤销/重开、连续两次冷启动和重载后立即关闭退出不恢复旧草稿通过，应用已退出。v4 亮暗各 1374 项呈现无差异，v5 未改显示样式。统一入口 `npm run test:markdown-operations`；真实 IME、Windows、真实拖拽/图床/多图片来源与长时压力仍未全面验收，不得称 64 组全组合通过。本任务未提交推送，期间外部提交到 c1eda93，接续重查 Git 并保护并行改动。

- 2026-09-12 空列表删除反馈横向排查：修复格式拆分/实体/换行与空项源码定位、行内源码收起的光标及跨片段选区恢复、前文长度变化后的同步/撤销定位。旧实现空项落回行内代码、正文选区落入引用定义均有失败对照；截图单次删除跳链接本轮未稳定还原，不得混同。61 核心 / 106 阅读集成（18 组结构操作）/ 144 组件与最终 macOS 包通过，最小及 163 行长文删除/输入/回车/向前删除/撤销保存逐字节恢复，测试文档关闭并退出应用。见[基本光标修复记录](docs/markdown-basic-caret-fix-2026-09-12.md)。保护并行粘贴/任务/代码行数修改，未提交推送；非文本结构状态栏仍有部分近似值，未新增 Windows/IME 验收。

- 2026-09-12 阅读“bug修复”后的独立基本操作检查：修复自动括号吞掉修饰键删除、已完成任务回车继承勾选、普通粘贴丢失首尾/重复空格。新增 8 组基本操作回归及真实浏览器、macOS 保存/撤销/重开证据，见[基本操作检查记录](docs/markdown-basic-operations-fix-2026-09-12.md)。完整阅读集成仍有行尾 HTML 光标误入引用定义失败，由并行“bug修复”继续处理，不能标全量通过；已修正代码实例测试把隐藏保留实例误计为新增的断言。独立测试应用已退出，未提交推送；保护并行光标与代码行号改动。

- 2026-09-12 用户截图反馈代码块行数不一致：已复现并修复围栏 token 终止换行被 Shiki 多算一行，只去掉一个显示用终止换行，保留真实空行。源码/预览/阅读代码块浏览器 5→6→撤销5 行及原文精确恢复通过；32 项专项、13 导出、47 组语法和生产构建通过。见[代码块行数记录](docs/markdown-code-lines-fix-2026-09-12.md)。未新增原生/Windows/IME 验收，未提交推送，保护并行阅读编辑修改。

- 2026-09-12 用户反馈阅读编辑连续 ↓ 回跳到 ` 水电`：已在用户补齐的原生长文中实际复现，修复 `inlineAt()` 仅凭非文本位置近似源码偏移误展开前一项代码的问题，增加真实文档光标范围校验。59 核心 / 103 阅读集成 / 144 组件与生产原生构建通过，同一 macOS 样例两组列表的下移及原文恢复核对通过，测试应用已退出。见[方向键回跳记录](docs/markdown-arrow-navigation-fix-2026-09-12.md)。非文本位置状态栏行列仍可能近似；本次未提交推送，未新增 Windows/IME 结论。

- 用户最新要求每轮优化先锁定范围，再复现验证、修复、测试、闭环，禁止随意发散。2026-09-12 已单独闭环[含行内链接正文的源码同步优化](docs/markdown-inline-link-performance-2026-09-12.md)：只改文档 reset 解析判定，保留上下文/结构变化回退。6,303 行同位置三次返回阅读中位数 797→481ms，解析 289→3.5ms；精确保存/撤销/重做、540 项呈现检查、58 核心 / 102 阅读集成及完整回归和构建通过。没有改输入法、配对、词典或其他产品功能，没有提交推送；不要据此扩展或宣称整体 Typora 验收完成。

- 2026-09-12 最新固定清单实施见[补齐与验收记录](docs/markdown-typora-implementation-2026-09-12.md)：阅读替换/链接/输入辅助、历史草稿、文末脚注、大纲、公式/旧式图表、代码选项、共享 CSS/隔离嵌入已接入；阅读实例跨模式保留，普通源码块局部解析，围栏等结构变化完整解析。55 核心 / 99 阅读集成 / 144 通用 / 119 XMind / 13 导出及其他回归通过，540 项复杂呈现对照无差异。6,303 行正文修改回阅读 508.8ms（局部解析 5.2ms），随后三轮 368–394ms；不是 IME 或 Typora 对跑。最新 macOS 包普通输入/跨模式/保存/撤销原文一致，测试文档已关闭、应用退出。真实 IME、Windows、系统/账户验收及格式符全套配对偏好、独立词典管理仍有明确边界；不得再按旧核对表把已实现项全部报缺失，亦不得标整体完成。本执行未提交推送，保护外部提交与 XMind 改动。
...
- CSV 已新增编辑 / 实时预览 / 阅读，复用共享模式控件；支持 CSV 表格、表头/分隔符选择及分页，模式与 Markdown/HTML 隔离并按编辑组独立保存。CSV 的 CRLF 源码投影比较已修正，模式往返不改原文。5 组 CSV（4 轮）/146 通用/8 分隔线/6 分屏/24 图表及生产构建通过，真实浏览器亮暗中英/720px、输入、键盘、分页与错误恢复通过。见 [CSV 实施与验收](docs/csv-view-modes-2026-09-20.md)。未重打原生包，未执行完整 test:all，模拟保存不计原生验证；未提交推送。
...
- 适用范围：仅限 DEditor 项目。按本地编辑器定位设计，默认在本地处理文件，不主动与外网交互，不擅自增加上传、遥测或外部服务依赖。
- 优先完整呈现用户文件并保留正常交互；不要套用公网服务的风险模型，因泛化的安全理由一刀切禁用脚本、表单、内嵌页面、本地资源或媒体控件，也不要静默删改用户源码和可见内容。
- 必要的隔离应限于保护主编辑器和文档状态的最小范围，优先使用独立预览文档，不能让正常渲染、编辑、保存和撤销失效。
- 新增限制必须对应具体问题和复现证据，并核对实际功能影响；不要根据假设主动收紧权限。修复时同时检查类似限制是否造成其他功能缺失。
...
## Markdown 预览性能接续（2026-09-12）

用户授权先实施三项建议后再核对剩余项，现已完成普通代码围栏有界高亮缓存、Mermaid 过期队列跳过、双向滚动位置索引。见 [实施、测量与剩余排序](docs/markdown-preview-performance-implementation-2026-09-12.md)。100 个代码块仅改正文暖渲染中位数 169.207→0.983ms；100 个废弃图表调用降为 0；真实浏览器 1002 标记稳定滚动不再全文扫描。已覆盖自定义 CSS 悬浮/聚焦只移动位置的失效边界。完整 test:all、perf:all（新增 perf:markdown-preview）、生产构建通过，复杂文档亮暗各 1374 项呈现对照无差异。剩余优先为长文重复坐标/历史扫描、后台预览重复生成，再评估 AST 偏移、大 DOM/主题及增量索引/图表复用；不代表已完成这些项目。未新增原生/真实 IME/Windows/长期压力验收，未提交推送，保护并行基本操作、保存及 XMind 工作。


## Markdown 输入与后台预览性能接续（2026-09-12）

用户要求继续优化，已完成 sourceChange 1024 字符分段比较、当前文档状态最多 32 个源码坐标查询缓存、隐藏 Markdown 预览延后生成。见 [实施与验收](docs/markdown-input-performance-2026-09-12.md)。75 万字符末尾插入历史差异 10.401→0.065ms；5000 段新投影五次重复定位 0.209→0.023ms；6 槽后台变化生成 9→1 次。完整 perf:all 独立通过，最终固定快照 test:all（62 核心/106 集成/144 通用/119 XMind及其他专项）和构建通过，6303 行浏览器输入/保存/跨模式撤销重做/续写/标签往返原文一致，亮暗各1374项呈现无差异。先前并发性能阈值及移动中任务Tab断言失败已在文档说明；不覆盖之后的并行改动。下一步须单独测长文首次坐标查找、AST偏移和输入至绘制成本，跨模式HTML与SVG复用仍未实施。未新增原生/真实IME/Windows/长期压力验收，本轮服务与测试页已关闭，未提交推送；保护并行列表/表格/保存及XMind修改。
