...
- 用户最新指令：全面测试、整体更新 README、更新上下文并提交推送，不再提问。Markdown 提交前已在独立暂存区快照完成完整自动化、性能和构建，详见 [提交前验证记录](docs/markdown-release-verification-2026-09-11.md)；源码原生历史事件捕获已修复。目标远程分支 `origin/feature/0.9.0`，独立 XMind 改动保持在原工作区，真实 IME 与双平台未验收项仍须补齐。
...
- 阅读编辑与实时预览已统一正文样式，并加入不写入原文的 H1–H6 提示。 用户随后要求按钉钉方式仅在光标进入标题时显示，现提示只跟随聚焦编辑器中的折叠文本光标，正文、非空选区、失焦及只读隐藏；不得恢复为全篇常显。见 [标题提示交互验证](docs/markdown-heading-hint-fix-2026-09-11.md)。旧测试页 split 误接阅读编辑的问题已修正；必须用真实 Preview 做同宽对照。另修复标题生成 id 导致撤销后空行变化、滚动清空重做历史。见 [呈现一致性验证](docs/markdown-presentation-fix-2026-09-11.md)。
