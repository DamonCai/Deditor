...
- 阅读编辑与实时预览已统一正文样式，并加入不写入原文的 H1–H6 提示。旧测试页 split 误接阅读编辑的问题已修正；必须用真实 Preview 做同宽对照。另修复标题生成 id 导致撤销后空行变化、滚动清空重做历史。见 [呈现一致性验证](docs/markdown-presentation-fix-2026-09-11.md)。
