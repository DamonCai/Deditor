...
## 1.0.2 整合提交验收（2026-09-24）

- 用户明确要求当前全部改动一起提交：长文/历史与Rust恢复性能、主题/目录复用、普通及无样式剪贴板、PlantUML重试按钮、测试和文档，以及同步的1.0.2版本信息。本轮完整工作区 `test:all` 与生产构建通过（108阅读集成、37剪贴板、147通用、119XMind等）；此前完整性能与隔离原生证据见下文记录，不扩展为最新全部改动的原生验收。
- 按用户要求统一本地提交，不推送；下方“未提交”是各阶段记录。首次拆分验证因隔离环境缺少Cargo路径停止，未产生提交；用户要求不拆分后在完整工作区重新测试通过。日志位于忽略目录 `tests/artifacts/performance-commit-2026-09-24/`。

## Rust 恢复与阅读视图性能（2026-09-24）

- 按用户保留现有交互、性能优先的要求，恢复快照改为版本化增量 IPC，Rust 每窗口共享文本分块，后台重建并调用原有原子写盘/草稿历史；失败不推进基线，丢失确认自动全量重同步。正文 TOC 复用未改行；主题切换保留编辑器/表格及内嵌光标、模式和历史。
- 同机三轮：千节标题输入368.36→116.44ms；主题就绪9500.08→162.37ms（优化后第一轮仍1539.86ms）；10个91.2万字符标签恢复前端准备106.07→0.17ms，IPC约27.36MB→1960B。Rust保留约22.43MiB编码文本缓存，首次原生全量同步38.66→113.58ms；首次打开无稳定收益，不称任意规模无卡顿。见[完整数据与代价](docs/markdown-native-performance-2026-09-24.md)。
- 111个前端→实际Rust→文件快照及元数据冻结、3Rust专项（含10000追加）、7目录/108阅读集成通过；完整test:all、串行perf:all、生产与独立macOS包通过。首轮旧IPC测试替身和并发期间文件搜索峰值失败均保留记录，修正/隔离后完整重跑。自建约70万字符原生阅读编辑/暗色/单次撤销重做/保存/已保存文件重启恢复逐字节核对通过；未新增未保存退出原生、Windows、IME或数小时内存结论。测试文档、应用和页面已关闭，未替换日常应用，未提交推送；并行剪贴板/PlantUML改动保留。

## 阅读编辑复制转义（2026-09-24）

- 后续补齐 Command/Ctrl+Shift+C 纯文本复制，Shift+V 从 pending 粘贴事件改为明确读取并字面插入；支持正文、表格、行内源码、内嵌代码及异步过期/只读/组合保护。37剪贴板、108集成与生产构建通过。浏览器 C 实际快捷键确认仅写 text/plain；自动化 Shift+V 不发送 keydown，V 仅记合成按键+真实剪贴板验证，未冒充原生物理键盘验收。详见下方横查记录末节。
- 普通复制/剪切改用可见文字，修复混合格式/跨段选区被上游序列化为 Markdown、泄漏反引号和反斜杠；显式“复制 Markdown”及富文本 HTML 保留。另修富文本与 TSV 将 emoji/图片 alt/行内公式/脚注当作换行的问题，不删除真实反斜杠。
- 32 组输入剪贴板（完整30+后补2）、108阅读集成、34表格、8图片剪贴板、通用回归/12分隔线及生产构建通过；浏览器亮暗快捷键/右键/显式Markdown复制、富文本粘回、剪切单次撤销通过。见[横查记录](docs/markdown-clipboard-escaping-2026-09-24.md)。测试页与服务已关闭；未重打原生包、未替换日常应用、未提交推送，保留已有并行改动。

## 阅读编辑长文与连续编辑性能（2026-09-24）

- 用户反馈大文档使用较久后卡顿。修复历史快照递归复制未改 AST、每次输入全量重建标题标记、目录以位置为 key 导致后续行整批重建；共享不可变原树、合并偏移并按需读取子树，保留原有 500 组历史、保存时机和文件字节，不增加内容限制。
- 同机三轮：253KB 文档 150 组历史额外保留堆 245.33→88.27MiB（-64.0%）；742KB/1000 节真实组件正文输入 128.86→81.70ms（-36.6%）；首标题输入目录重建 1028→1 行。标题仍约 304ms，首次打开/主题与重新挂载仍慢；不称全部流畅或已验原生长期泄漏。见[完整对比与边界](docs/markdown-reading-performance-2026-09-24.md)。
- 20,000 次偏移、96 次混合源码/定位、150/600 次历史及 10,000 段边界通过；完整 `test:all`、`perf:all`、生产与独立 macOS debug 包构建通过。浏览器千节输入/保存快照/单次撤销重做、暗色与双标签历史逐字核对通过；不是原生写盘/IME/Windows 验收。测试页与服务已关闭，测试包仅构建、未启动；未替换日常应用，未提交推送，保留原有 PlantUML 超时改动。
...
- 普通修复包：有足够尾部空间的目录目标定位保持顶部，保存/一次撤销/重做/重开字节一致；XMind 逐键 nihao+Space 输入你好、Enter 保存、一次撤销/重做/重开及后续取消逐归档核对通过。测试文档关闭、应用退出。见[本轮整合](docs/native-ime-final-2026-09-22.md)。
- 左右 Command 显式写法仍先把 s 送入组合再产生 meta-keydown；系统截图快捷键也先写入字符，Screenshot/Preview连接超时，候选窗不可观察。不能用这些结果声称 IN-03/04/05/06 已验收；已询问真实键盘结果，尚无回复。当前仍8类29项，中文4项为实机证据缺口。本轮本地提交，不推送、不替换日常应用。

## 中文输入专项接续（2026-09-22，4 项仍需实机证据）

- 用户要求三个 agent 集中清理中文输入。新增4组真实阅读组件+saveFile竞态回归，覆盖保存未提交候选、慢写盘继续选词、保存后取消、同任务组合结束，接入test:all。普通隔离包实际File→Save保存ni后Space继续选你、dirty/再保存/单次撤销重做/重开逐字节通过，见[保存核查](docs/native-composition-save-2026-09-22.md)。没有确定产品缺陷，不添加输入过滤或强制结束composition。
- IN-03/04/06仍缺系统候选窗视觉定位；系统SCIM扩展CUA返回noWindowsAvailable，TextInputMenuAgent超时，CursorUIViewService虽可连接但AX和截图为空白，不冒充候选证据。Markdown官方WK异步caret坐标机制和XMind2736几何组合审查未发现确定缺陷；这些不是原生候选窗验收。IN-05仅剩物理Command+S前置输入序列，不能用菜单点击替代。
- 已向用户请求物理键盘候选窗/快捷键的实际观察，等待结果，不将未回答计为通过。待办仍8类29子项，中文4项未假清空。自建文档已保存关闭，隔离测试应用退出；只提交回归和记录，不推送、不替换日常应用。

## 两类紧急验收继续推进（2026-09-22）

- 用户要求排除 Windows、选两个紧急类别，三个 agent 并行推进原生中文输入和 macOS 桌面。IN-01 按原动作允许的有界复现结论收口：三次原生批量/分批格式输入保存重开无额外闭合符，19 组配对专项通过；历史首次异常仍未定因，不称已修复该次异常。见[后续输入核查](docs/native-input-followup-2026-09-22.md)。当前待办 **8 类、29 子项**。
- 真实 composition 中滚动0→800保持原插入位置，Space提交“你”；保存/单次撤销/重做/重开字节一致。左右移动的`n i`由系统预编辑事件提供，Escape取消精确恢复；IN-03/04只保留候选窗口视觉定位。组词中Cmd+S仍在keydown前先输入字母，普通WK对照同型，未加过滤掩盖。见[组合输入证据](docs/ime-implementation-audit-2026-09-22.md)。
- Dock替代入口Control-F3实际进入WK输入框，连接仍超时；Finder桌面截图不含其他应用，现有CUA缺全局frame/跨应用目标坐标依据，MAC-01/02/03仍未验收。新增仅用于自建样例的组合事件、原生拖放/窗口归属观察，无产品源码改动，诊断不进入正式包。见[桌面记录](docs/native-desktop-acceptance-2026-09-22.md)。测试文档已保存关闭、测试实例退出、输入源恢复简体拼音；未替换日常应用，仅本地提交不推送。

## 原生输入、分隔线与全屏继续收尾（2026-09-22）

- 三 agent 继续并行，原 8 类拆为 34 个可独立验收子项，本轮完成 IN-02、CLIP-04、MAC-04、MAC-05，当前 **8 类、30 子项**；分类数不表示未实现功能数。以 [当前待办](docs/pending-tasks.md) 为准，完成项已移除。
- Markdown / XMind 真实逐键拼音加 Space 提交“你好”、取消未提交拼音、保存、撤销重做和重开均核对通过。候选窗视觉定位未完成；组词 Cmd+S 的字母进入组合文字在普通 WK textarea 也复现，不过滤合法输入掩盖现象。见 [输入验收](docs/native-input-acceptance-2026-09-22.md)。
- 原生主预览拖动命中且成功捕获后，buttons=0 使旧逻辑提前结束；仅对起始零按钮且持续持有捕获的手势放行，保留正常鼠标丢键退出和所有清理路径。12 专项、147 通用、生产与隔离 macOS 构建通过；普通最终包正反改宽、选择保护释放、保存字节一致通过。见 [分隔线闭环](docs/native-pane-resize-diagnostic-2026-09-22.md)。
- 全屏菜单改为状态中立的“切换全屏 / Toggle Full Screen”。最终前端副本仅加只读窗口观测，实际双窗全屏进退、窗口菜单焦点往返、独立输入保存/撤销与三文件基线逐字节核对通过；不扩大为逐帧动画或多显示器。见 [桌面验收](docs/native-desktop-acceptance-2026-09-22.md)。自建文档保存关闭、全部测试应用退出；诊断代码不进入正式包，未替换日常应用，按既有授权仅本地提交、不推送。
...
## 原生输入、剪贴板与桌面并行验收（2026-09-22）

- 三agent准备与审查，主端独占原生界面。真实Excel往返发现并修复额外空表头及已有表格粘贴拆表；保留真实空首行、单元格格式与周边行。修复包Excel 4×4往返、2×2覆盖既有表格及保存/单次撤销/重做/重开精确通过，Word粗斜体/链接/中文emoji往返通过。26剪贴板/34表格/107阅读集成、生产与隔离macOS包构建通过。详见[原生剪贴板](docs/native-clipboard-drag-2026-09-22.md)。
- 拼音逐键n/i两次仅见ni无候选；Dock仍连接超时；Finder真实拖放、跨Space全屏与持续拖选未验；主预览分隔线x614及x612拖动未取得宽度变化与原生命中证据。不归因工具/IME，不计通过，见[输入](docs/native-input-acceptance-2026-09-22.md)与[桌面](docs/native-desktop-acceptance-2026-09-22.md)。当前仍8类待验收，已完成Excel/Word具体范围从待办移除。
- 自建测试文档保存关闭，隔离DEditor、Excel、Word退出，简体拼音保留；未改用户文档、未替换日常应用，按既有授权本地提交不推送。本轮未跑全量test:all/perf:all，不新增Windows/真实候选结论。
