...
用户再次要求处理后，Review23 冷启动并打开同一自建 IME 样例，点击主题、F2 进入文字编辑成功；逐键 n/i 仍得到英文 `ni`。系统 `TextInputMenuAgent` 入口返回 `timeoutReached`，画面捕获返回 ScreenCaptureKit -3811。Ctrl+Space 后仍只有英文和字面空格，未取得中文候选浮窗，不能计为通过。退出编辑后有未保存测试输入，关闭该自建文档并选择 Don't Save，返回 Untitled 后通过应用菜单退出。磁盘样例与原始归档 SHA-256 均为 `4bddf1e0c6885359c2275e72be14334a5e59b3e3ea2bc34836f4790d7c3d594e`；进程检查确认 Review23 与原版 XMind 均已退出。本次没有启动原版。

当前 Git 工作区在接续时已干净，HEAD 为 `b5ad9e5`，这是接续前已有状态，本任务本次没有提交或推送。Review23 清单仍为 7/8 文件匹配，仅共享组件回归脚本不同；XMind 运行时、核心测试及样例匹配。未因环境重试改动运行时代码。当前工具仅提供本机桌面控制，没有可用 Windows 验收环境；剩余两项继续明确保留为未验证。

## 系统输入源检查与剩余验收准备

用户继续要求处理后，通过系统设置实际查看了键盘、输入法及快捷键：ABC、简体拼音和搜狗拼音已配置；输入法菜单显示开启，Ctrl+Space / Ctrl+Alt+Space 均启用。没有修改这些系统配置。重新初始化控制会话、Raise 测试窗口、使用明确的 Control_L / Alt_L 组合后仍是英文直输；此次截图恢复，显示英文 `Ni` 自动纠正建议，不能当成中文候选列表。系统中文输入法扩展入口返回 NSCocoaErrorDomain 256 / NSOSStatus -10814，搜狗输入法控制返回 timeoutReached，系统菜单键盘入口未取得候选窗口。

所有本轮输入均为自建 IME 文档；切换应用引发失焦提交的测试文字亦在关闭时选择 Don't Save 丢弃。返回 Untitled 后退出 Review23；本次打开的系统设置也通过应用菜单退出。进程检查确认 Review23、原版 XMind 和系统设置均未运行，IME 文件仍与原始副本精确相同。本次没有启动原版 XMind。

补齐了不依赖原生环境的接续准备：

- `scripts/xmind-native-acceptance.ts` 生成六组全新自建基线/工作副本/精确预期，覆盖标题、真实 IME 输入目标、四类组合字符标签、左右深层混合折叠鱼骨、多表和附件；目录含中文、空格与井号。拒绝覆盖已有结果。
- 同一入口的 `check` 检查保存结果仅含预期修改，其余 JSON、ZIP 条目、附件精确保留；输出明确区分归档通过与尚未评估的原生 UI / 中文候选。
- `tests/xmind-native-acceptance.conf.json` 使用独立 identifier，无文件关联，供 Windows 无安装构建。实际流程见 [原生验收步骤](xmind-native-acceptance.md)。此配置尚未在 Windows 构建，不计 Windows 验收。
- 新脚本 TypeScript 检查通过；六组经生产归档写入器生成的正确修改均通过校验，六份未修改输入和三份故意损坏/多改字段/缺失资源的归档全部被拒绝，重复生成到原目录也被拒绝。证据在 `tests/artifacts/xmind-native-acceptance-20260912/verifier-tests/results.json`，这些是校验器验证，不能计为六组原生测试通过。

本次未改 XMind 运行时代码，不重复运行未变化的既有完整回归；未提交、推送或安装正式应用。当前仍缺真正的中文候选位置证据和 Windows 原生执行环境，不能标记整体完成。
