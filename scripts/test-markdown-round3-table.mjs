...
try {
 ...
 await test('T07 generated table between paragraphs accepts horizontal entry at both boundaries',async()=>{
  await reset('Before');await select('Before');await key('Enter');await act(async()=>app.getVisualEditor().insert('| | |\n| --- | --- |\n| | |',true));await cell(0,0);await input('header');await cell(1,1);await input('last');await key('Enter',{ctrlKey:true});await input('After');const before=content();await select('Before');await key('ArrowRight');assert.equal(currentCell(),cellAt(0,0));await select('After',0);await key('ArrowLeft');assert.equal(currentCell(),cellAt(1,1));assert.equal(view.state.selection.$from.parentOffset,4);assert.equal(content(),before);await input('X');assert.equal(matrix()[1][1],'lastX');await durable();
 });
 await test('T08 generated final table exits by Mod Enter and allows paragraph editing after history',async()=>{
  await fresh();await cell(1,1);await input('last');await key('Enter',{ctrlKey:true});assert.equal(view.state.selection.$from.depth,1);await input('after');const edited=content();await history();await history(true);assert.equal(content(),edited);await input('X');assert.equal(matrix()[1][1],'last');assert.match(content(),/afterX\n?$/);await durable();
 });
