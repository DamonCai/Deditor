...
const start=()=>{moves=[];const e=event('pointerdown');const stop=beginPaneResize(handle,e,e=>moves.push(e.clientX));assert.equal(e.defaultPrevented,true);assert.equal(captured,7);assert.ok(doc.body.classList.contains('deditor-pane-resizing'));assert.ok(handle.classList.contains('dragging'));return stop;};
