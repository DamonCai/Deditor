...
await test('B04 sole table last cell Mod Enter keeps table and permits following prose',async()=>{
 const original='| A | B |\n| --- | --- |\n| C | D |\n';for(const before of [false]){await reset(original);await select('D',1);await key('Enter',{ctrlKey:true});assert.equal(view.state.selection.$head.depth,1);assert.equal(view.state.selection.$head.parent.type.name,'paragraph');await writeWord('outside');assert.ok(before?content().startsWith('outside'):content().endsWith('outside'));await undo();await undo();assert.equal(content(),original);await redo();await redo();await persistRemount();}
});
