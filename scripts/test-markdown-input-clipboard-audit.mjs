...
try {
 await test('UX vertical alignment survives rich copy, paste, editing and one undo',async()=>{
  ...
  assert.match(payload.html,/vertical-align: middle/);assert.match(payload.html,/vertical-align: bottom/);assert.doesNotMatch(payload.text,/deditor:valign/);
  await reset('');await paste(payload.text,payload.html);const before=content();assert.match(before,/deditor:valign=middle/);assert.match(before,/deditor:valign=bottom/);
  await select('bold');await act(async()=>view.dispatch(view.state.tr.insertText('X').setMeta('deditor-list-operation',true)));assert.match(content(),/boldX/);assert.match(content(),/deditor:valign=middle/);await exactHistory(before);
 });
 await test('C09 explicit copy formats preserve source, selected table bounds and rich marks',async()=>{
  const original='Before\n\n| A | B |\n| --- | --- |\n| one<br>two | **bold** |\n\nAfter\n';
  await reset(original);await select('Before');let payload=app.getVisualEditor().clipboard();assert.equal(payload.markdown,original);assert.match(payload.html,/<strong>bold<\/strong>/);assert.doesNotMatch(payload.html,/text-align/);assert.equal(payload.tsv,undefined);assert.equal(content(),original);
  await select('bold');payload=app.getVisualEditor().clipboard();assert.equal(payload.tsv,'A\tB\n"one\ntwo"\tbold');
  const {CellSelection,TableMap}=await import('@milkdown/kit/prose/tables');let table,pos;view.state.doc.descendants((n,p)=>{if(n.type.name==='table'){table=n;pos=p;}});const map=TableMap.get(table);
  await act(async()=>view.dispatch(view.state.tr.setSelection(CellSelection.colSelection(view.state.doc.resolve(pos+1+map.map[1])))));
  payload=app.getVisualEditor().clipboard();assert.equal(payload.tsv,'B\nbold');assert.match(payload.markdown,/\*\*bold\*\*/);assert.doesNotMatch(payload.markdown,/one|Before|After/);assert.equal(content(),original);
  await reset('');await paste(payload.text,payload.html);assert.doesNotMatch(content(),/\| :/);assert.match(content(),/\*\*bold\*\*/);
  const written=[];Object.defineProperty(navigator,'clipboard',{configurable:true,value:{writeText:async text=>written.push(text)}});
  await app.writeMarkdownClipboard(payload,'markdown');await app.writeMarkdownClipboard(payload,'spreadsheet');assert.deepEqual(written,[payload.markdown,payload.tsv]);
  const previous=document.execCommand;let rich;
  document.execCommand=()=>{const data=new Map(),event=new dom.window.Event('copy',{cancelable:true});Object.defineProperty(event,'clipboardData',{value:{setData:(k,v)=>data.set(k,v)}});document.dispatchEvent(event);rich=data;return true;};
  try{await app.writeMarkdownClipboard(payload,'rich');assert.equal(rich.get('text/html'),payload.html);assert.equal(rich.get('text/plain'),payload.text);}finally{document.execCommand=previous;}
 });
 await test('C10 literal paste bypasses markup and table detection with separate undo; stale async paste is rejected',async()=>{
  for(const original of ['target\n','| A | B |\n| --- | --- |\n| target | keep |\n']){
   await reset(original);await range('target',0,6);const literal='**literal**\n# heading\nleft\tright';
   Object.defineProperty(navigator,'clipboard',{configurable:true,value:{readText:async()=>literal}});
   await act(async()=>assert.equal(await app.getVisualEditor().pastePlain(),true));
   assert.ok(visible().includes(literal));assert.equal(view.dom.querySelector('strong'),null);assert.equal(view.dom.querySelector('h1'),null);await exactHistory(original);
  }
 ...
 await test('Q01 slash filters and inserts existing block types in one undoable operation',async()=>{
  for(const [query,type] of [['table','table'],['code','code_block'],['mermaid','code_block'],['plantuml','code_block'],['task','bullet_list'],['bullet','bullet_list'],['heading','heading'],['quote','blockquote'],['divider','hr']]){
   await reset('before\n\nafter\n');await select('before');await key('Enter');
   for(const ch of '/'+query)await input(ch);
   const menu=document.querySelector('.md-quick-insert');assert.ok(menu,query);assert.ok(menu.querySelector('button'),query);const before=content();
   await key('Enter');assert.ok(view.state.doc.content.content.some(n=>n.type.name===type),query+': '+JSON.stringify(view.state.doc.toJSON()));assert.ok(content().endsWith('after\n'));assert.equal(document.querySelector('.md-quick-insert'),null);await exactHistory(before);
  }
 });
 await test('Q02 slash dismissal, nonmatching query and nested blocks retain ordinary input',async()=>{
  await reset('before\n');await select('before');await key('Enter');await input('/');assert.ok(document.querySelector('.md-quick-insert'));
  await key('ArrowDown');assert.match(document.querySelector('[aria-selected=true]').textContent,/Code block/);await key('Escape');assert.equal(document.querySelector('.md-quick-insert'),null);await input('notfound');assert.equal(document.querySelector('.md-quick-insert button'),null);await key('Enter');assert.ok(content().includes('/notfound'));
  for(const source of ['text /table\n','| A |\n| --- |\n| /table |\n','- /table\n','```text\n/table\n```\n']){await reset(source);await select(source.startsWith('text')?'text /table':'/table');assert.equal(document.querySelector('.md-quick-insert'),null);assert.equal(content(),source);}
 });
 await test('C05 whole-document copy and rich paste preserve mixed structure, source and independent history',async()=>{
  const original='# 开始\n\n正文 **加粗** 与 [链接](https://example.com/path)\n\n> 引用\n\n- 父项\n  - 子项\n\n- [x] 完成\n- [ ] 待办\n\n| 甲 | 乙 |\n| :--- | ---: |\n| 一<br>二 | **三** |\n\n```js\nconst value = 1;\n```\n\n结束\n';
  await reset(original);const expected=semantics();await selectDocument();const copied=await copySelection();
  assert.equal(content(),original,'copy never changes the source');assert.match(copied.text,/开始/);assert.match(copied.text,/结束/);
  await reset('');await paste(copied.text,copied.html);view.state.doc.check();
 ...
 await test('C07 whole-document cut restores exact source and selection, then accepts new typing',async()=>{
  for(const original of ['第一段\n\n最后一段\n','| 唯一 |\n| --- |\n| 内容 |\n','```js\nconst x = 1;\n```\n']){
   await reset(original);await selectDocument();const copied=await copySelection('cut');
   assert.equal(view.state.doc.textContent,'','cut removes the entire selection');const empty=content();
   await act(async()=>app.markdownHistory());assert.equal(content(),original);assert.equal(view.state.selection.from,0);assert.equal(view.state.selection.to,view.state.doc.content.size);
   await act(async()=>app.markdownHistory(true));assert.equal(content(),empty);
   await input('继续');assert.equal(view.state.doc.textContent,'继续');
   await act(async()=>app.markdownHistory());assert.equal(content(),empty,'continued typing is separate from cut');
   await paste(copied.text,copied.html);assert.ok(view.state.doc.textContent.length>0);await exactHistory(empty);
  }
 });
 await test('C08 full-copy raw blocks retain source rather than rendered HTML on paste',async()=>{
  for(const block of ['<details>\n<summary>标题</summary>\n正文 **原文**\n</details>','```html\n<svg viewBox="0 0 10 10"><rect width="10" height="10" /></svg>\n```','---\ntitle: 自建\n---']){
   const original=block+'\n\n尾段\n';await reset(original);const expected=semantics();await selectDocument();const copied=await copySelection();
   await reset('目标将被替换\n');await selectDocument();await paste(copied.text,copied.html);
   assert.deepEqual(semantics(),expected);await exactHistory('目标将被替换\n');
  }
 });
 ...
 assert.equal(runtimeErrors.length,0);console.log(`${passed} input/clipboard audit groups passed`);assert.deepEqual(failedCases,[],'failed clipboard groups');
