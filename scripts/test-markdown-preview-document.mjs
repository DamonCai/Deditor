...
const renderedFence=document.createElement('div');renderedFence.innerHTML=markdownDisplayHtml(raw);assert.ok(renderedFence.querySelector('.html-render-block svg rect'));assert.equal(renderedFence.querySelector('.html-render-block script'),null);assert.equal(renderedFence.querySelector('.html-render-block rect').hasAttribute('onclick'),false);assert.equal(renderedFence.querySelector('.html-render-block pre'),null);
for(const html of [raw,'&lt;img src=x onerror=alert(1)&gt; &amp; literal','<p data-line="9" onclick="alert(1)">safe<script>bad()</script></p><iframe src="https://example.com" srcdoc="bad"></iframe><a href="file:///tmp/a.md">file</a>','<div data-deditor-preview-line="98"><p data-line="7">x</p></div>','<table><tbody><tr><td>a</td></tr></tbody></table><!-- comment --><p>tail</p>']){
 ...
 const expected=document.createElement('div');expected.innerHTML=markdownDisplayHtml(html);for(const el of expected.querySelectorAll('[data-deditor-preview-line]'))el.removeAttribute('data-deditor-preview-line');
