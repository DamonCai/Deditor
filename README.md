...
阅读编辑与实时预览共用正文排版：标题、段落、列表、引用、表格、代码、公式及图片在相同栏宽和字号下保持一致，字号调整同时生效。阅读编辑仅在编辑光标进入标题时，在该标题左侧显示轻量 H1–H6 级别提示；移入正文、选中文字或失去编辑焦点后隐藏。提示不占正文空间、不写入 Markdown，在只读和导出中隐藏。
