...
阅读编辑与实时预览共用正文排版：标题、段落、列表、引用、表格、代码、公式及图片在相同栏宽和字号下保持一致，字号调整同时生效。阅读编辑在页边显示当前编辑块的轻量提示：标题 H1–H6、正文 `¶`、引用 `>`、提示块 `[!]`、脚注 `[^]`、列表 `-` / `1.`、任务 `[ ]`、表格 `| |`、代码 `</>`、公式 `$$`、图表 UML、图片 IMG、分隔线 `---`，以及 HTML / YAML / 保留原文 MD。提示跟随编辑光标或整块选中位置，选择一段文字、失焦或切到只读时隐藏；不占正文空间、不写入 Markdown，也不会出现在实时预览或导出中。
...
- 图片存放目录：默认 `assets`，可改为文档目录下的相对子目录；源码粘贴和阅读编辑上传共用设置。文档 YAML 中的 `typora-copy-images-to` 优先，可使用 `./${filename}.assets`；只接受文档目录下的相对子目录，非法值回退到全局设置。
