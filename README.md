...
阅读编辑期间，隐藏的源码编辑器在切回源码时一次同步最新内容，减少两套编辑器重复处理。保存与跨模式撤销仍使用同一份当前文档。复杂长文测量与已验证范围见[性能优化记录](docs/markdown-performance-2026-09-12.md)。
...
| 原生后端 | `src-tauri/src/lib.rs`：文件 IO、搜索、路径处理、原生菜单、日志等；`markdown_images.rs`：Markdown 图片下载与 PicGo 通信 |
