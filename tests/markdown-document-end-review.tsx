...
function Review() {
  ...
  return <div style={{height:'100vh',display:'flex',flexDirection:'column'}}>
    <nav>{[initial,long].map((text,i)=><button key={i} onClick={()=>{useEditorStore.getState().setContent(text,'focus-review','command');setBaseline(text);}}>{i?'长行样例':'文末代码样例'}</button>)}
