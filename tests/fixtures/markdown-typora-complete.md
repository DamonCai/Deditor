...
语法简写定位点：==高亮 **重点**==、H~2~O、X^2^、:smile: 与 :rocket:。代码 `==不转换== :smile:` 和转义 \:smile: 保留。
...
行内化学 $\ce{H2O}$ 与单位 $\pu{1.2e3 kJ}$。

$$
E = mc^2 \label{energy}
$$

公式引用 $\eqref{energy}$，普通引用 $\ref{energy}$。
...
participant 编辑器
用户->编辑器: 输入内容
Note right of 编辑器: 保留原文\n检查撤销
编辑器-->用户: 保存成功
```

## 替换与边界

**item12**、item34、Cat、cat、scatter、中文词、中文、😀。
