...
const fixtures: Record<string,string> = {
 ...
 '链接脚注': '# Destination\n\n[Jump](#destination) and [reference][guide] with note[^n]. repeated[^n].\n\n[^n]: **Rich** and `code`.\n\n    second paragraph\n\n[guide]: https://example.com\n',
 '扩展格式': 'alpha ==highlight== H~2~O x^2^ :smile: omega\n\n`==literal== :smile:`\n',
 '正文': '前段保留。\n\nalpha beta\n\n后段保留。\n',
 '任务': '- [x] completed\n- [ ] pending\n\n后段保留。\n',
 '列表': '- alpha\n- beta\n- gamma\n\n后段保留。\n',
 '表格': '| A | B |\n| --- | --- |\n| one | two |\n\n后段保留。\n',
...
function Review() {
 ...
 React.useEffect(()=>{const update=()=>{const s=document.getSelection();setSelection(JSON.stringify({anchor:s?.anchorNode?.textContent?.slice(0,160),anchorOffset:s?.anchorOffset,focus:s?.focusNode?.textContent?.slice(0,160),focusOffset:s?.focusOffset}));};document.addEventListener('selectionchange',update);return()=>document.removeEventListener('selectionchange',update);},[]);
