...
import MarkdownVisualHost from "../src/components/MarkdownVisualHost";
...
useEditorStore.setState({tabs:params.has("cold")?[]:[{id:"search-review",filePath:path,content:original,savedContent:original}],activeId:params.has("cold")?null:"search-review",workspaces:["/generated"],language:en?"en":"zh",theme:dark?"dark":"light",markdownMode:params.has("source")?"source":"visual",autoSave:"off"});
...
function Review(){
 ...
 return <div style={{height:"100vh",display:"flex",flexDirection:"column"}}><div style={{display:"flex",gap:12,padding:8}}><Button onClick={()=>setOpen(true)}>搜索 / Search</Button><Button onClick={()=>useEditorStore.setState({markdownMode:mode==="visual"?"source":"visual"})}>切换模式 / Mode</Button><span role="status">{mode} · {!activeId?"尚未打开文件":content===original?"原文一致":"已修改"}</span></div><div style={{flex:1,minHeight:0,position:"relative",display:"flex"}}>{activeId && readyId!==activeId?<span>正在加载测试编辑器 / Loading test editor</span>:mode==="visual"?<MarkdownVisualHost activeId={activeId} active theme={dark?"dark":"light"}/>:<EditorHost activeId={activeId} theme={dark?"dark":"light"} fontSize={14}/>}</div>{open && <FindInFiles open onClose={()=>setOpen(false)}/>}</div>;
}
